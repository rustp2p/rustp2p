...
impl TunnelDispatcher {
    pub async fn new(config: Config) -> io::Result<TunnelDispatcher> {
        Box::pin(Self::new_impl(config)).await
    }
    pub(crate) async fn new_impl(mut config: Config) -> io::Result<TunnelDispatcher> {
        ...
        Ok(Self {
            ...
            recv_buffer_size,
            node_context,
            tunnel_dispatcher: tunnel_factory,
            shutdown_manager,
            active_punch_sender,
    ...
    pub(crate) fn tunnel_router(&self) -> TunnelRouter {
        TunnelRouter {
            ...
            socket_manager: self.tunnel_dispatcher.socket_manager(),
            shutdown_manager: self.shutdown_manager.clone(),
        }
    }
}

impl Drop for TunnelDispatcher {
    fn drop(&mut self) {
...
impl TunnelDispatcher {
    pub(crate) async fn dispatch(&mut self) -> io::Result<Tunnel> {
        ...
        Ok(Tunnel {
            ...
            node_context: self.node_context.clone(),
            tunnel,
            tunnel_transmit: self.tunnel_router(),
            route_table: self.route_table.clone(),
...
impl TunnelRouter {
    ...
    async fn send_broadcast_impl(&self, buf: &[u8], group_code: &GroupCode, src_id: &NodeID) {
        ...
        let buf_bytes = bytes::Bytes::copy_from_slice(buf);
        for (owner_id, (list, route)) in map {
            if list.len() <= 1 && route.is_direct() {
                if let Err(e) = self
                    .socket_manager
                    .send_to(buf_bytes.clone(), &route.route_key())
                    .await
                {
    ...
    pub(crate) async fn send_packet_to_route(
        ...
        if let Some(src_id) = self.node_context.load_id() {
            ...
            if let Some(route_key) = route_key {
                return self
                    .socket_manager
                    .send_to(packet.into_buf().into(), route_key)
                    .await;
            }
            self.send_to_impl(packet.into_buf().into(), &group_code, &src_id, dest_id)
                .await
        } else {
    ...
    pub(crate) fn try_send_packet_to_route(
        ...
        if let Some(src_id) = self.node_context.load_id() {
            ...
            if let Some(route_key) = route_key {
                return self
                    .socket_manager
                    .try_send_to(packet.into_buf().into(), route_key);
            }
            self.try_send_to_impl(packet.into_buf().into(), &group_code, dest_id)
        } else {
            Err(io::Error::other("no id specified"))
        }
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn try_kcp_send_to<D: Into<NodeID>>(&self, buf: &[u8], dest: D) -> io::Result<()> {
        let mut send_packet = self.allocate_send_packet();
        send_packet.set_payload(buf);
    ...
    pub(crate) fn allocate_send_packet(&self) -> SendPacket {
        let buf = BytesMut::with_capacity(self.send_buffer_size);
