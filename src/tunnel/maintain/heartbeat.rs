...
pub async fn heartbeat_loop(tunnel_tx: TunnelTransmit, heartbeat_interval: Duration) {
    let mut count = 0;
    loop {
        if count % 3 == 2 {
...
async fn heartbeat_request(tunnel_tx: &TunnelTransmit) -> io::Result<()> {
    let mut packet =
...
async fn timestamp_request(tunnel_tx: &TunnelTransmit) -> io::Result<()> {
