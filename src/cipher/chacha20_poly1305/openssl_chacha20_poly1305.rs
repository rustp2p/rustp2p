...
impl ChaCha20Poly1305Cipher {
    pub fn decrypt(&self, extra_info: [u8; 12], payload: &mut [u8]) -> io::Result<usize> {
        ...
        // 设置标签
        decrypter.set_tag(tag).map_err(|e| io::Error::new(
            io::ErrorKind::InvalidData,
            format!("ChaCha20Poly1305 decryption failed: {e}"),
        ))?;
        
        // 解密数据
        let mut decrypted = vec![0; data_len - ENCRYPTION_RESERVED + Cipher::chacha20_poly1305().block_size()];
        let mut count = decrypter.update(
            &payload[..data_len - ENCRYPTION_RESERVED],
            &mut decrypted,
        ).map_err(|e| io::Error::new(
            io::ErrorKind::InvalidData,
            format!("ChaCha20Poly1305 decryption failed: {e}"),
        ...
        count += decrypter.finalize(&mut decrypted[count..]).map_err(|e| io::Error::new(
            io::ErrorKind::InvalidData,
            format!("ChaCha20Poly1305 decryption failed: {e}"),
        ))?;
    ...
    pub fn encrypt(&self, extra_info: [u8; 12], payload: &mut [u8]) -> io::Result<()> {
        ...
        // 加密数据
        let mut encrypted = vec![0; data_len - ENCRYPTION_RESERVED + Cipher::chacha20_poly1305().block_size()];
        let mut count = encrypter.update(
            &payload[..data_len - ENCRYPTION_RESERVED],
            &mut encrypted,
        ).map_err(|e| io::Error::new(
            io::ErrorKind::InvalidData,
        ...
        count += encrypter.finalize(&mut encrypted[count..]).map_err(|e| io::Error::new(
            io::ErrorKind::InvalidData,
            format!("ChaCha20Poly1305 encryption failed: {e}"),
        ))?;
        ...
        encrypter.get_tag(&mut tag).map_err(|e| io::Error::new(
            io::ErrorKind::InvalidData,
            format!("ChaCha20Poly1305 encryption failed: {e}"),
        ))?;
